...
void loop() {
  ...
  if (packetSize)
  {
    ...
    String newdata = incomingPacket;
    //char StringMode[] = "MODE="; 
    //Serial.println(newdata.indexOf("MODE="));
    
    if(newdata.indexOf("MODE=") == 0){
      
      //Serial.print("Set Mode:");
      newdata.remove(0, 5);
      //Serial.println(newdata);
      //uint8_t next_mode = atoi (newdata.substring(0, 3).c_str ());   
      if(ws2812fx.getMode()!=newdata.toInt()){
        ws2812fx.setNumSegments(1);
        ws2812fx.setLength(LED_COUNT);
        ws2812fx.setMode(newdata.toInt());
        Serial.print("Mode is:");
        Serial.println(ws2812fx.getMode());
      }else{
        Serial.print("Mode is already:");
        Serial.println(ws2812fx.getMode());
      }
      
    }
    if(newdata.indexOf("SPEED=") == 0){
      //Serial.print("Set Speed:");
      newdata.remove(0, 6);
      //Serial.println(newdata);
      //uint8_t next_speed = atoi (newdata.substring(0, 5).c_str ());  
      //Serial.println(next_speed);
      if(ws2812fx.getSpeed()!=newdata.toInt()){
        ws2812fx.setSpeed(newdata.toInt());
        Serial.print("Speed is:");
        Serial.println(ws2812fx.getSpeed());
      }else{
        Serial.print("Speed is already:");
        Serial.println(ws2812fx.getSpeed());
      }
    }
